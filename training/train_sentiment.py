...
def main() -> None:
    progress = StageProgress("sentiment", 3)
    dataset_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("data/query_labels.csv")
    progress.advance("loading training rows")
    rows = sample_training_rows_for_label(dataset_path, "sentiment_label", 50000)
    progress.note(f"loaded {len(rows)} rows")
    progress.advance("fitting model")
    fit_progress = ProgressBar("sentiment:fit", estimate_text_classifier_fit_steps(len(rows)), every=1)
    ...
    classifier = SingleLabelTextClassifier.build_from_records(rows, "sentiment_label", fit_progress_callback=on_fit_batch)
    fit_progress.finish("fit complete")

    model_dir = Path("models")
    model_dir.mkdir(exist_ok=True)
    progress.advance("writing artifact")
    dump(classifier.model, model_dir / "sentiment.joblib")
    print(json.dumps({"trained_samples": len(rows), "output": str(model_dir / "sentiment.joblib")}, ensure_ascii=False))
