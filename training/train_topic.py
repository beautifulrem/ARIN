...
from query_intelligence.nlu.classifiers import TOPIC_SAMPLES, MultiLabelClassifier, estimate_text_classifier_fit_steps
from query_intelligence.training_data import sample_training_rows_for_label

MAX_TOPIC_ROWS = 50000
...
def main() -> None:
    progress = StageProgress("topic", 3)
    ...
    if dataset_path and dataset_path.exists():
        rows = sample_training_rows_for_label(dataset_path, "topic_labels", MAX_TOPIC_ROWS)
    else:
        rows = [{"query": text, "topic_labels": labels} for text, labels in TOPIC_SAMPLES]
    progress.note(f"loaded {len(rows)} rows")
    progress.advance("fitting model")
    fit_progress = ProgressBar("topic:fit", estimate_text_classifier_fit_steps(len(rows)), every=1)
    ...
    classifier = MultiLabelClassifier.build_from_records(rows, "topic_labels", fit_progress_callback=on_fit_batch)
    ...
    dump(
        ...
        model_dir / "topic_ovr.joblib",
    )
    print(json.dumps({"trained_samples": len(rows), "output": str(model_dir / "topic_ovr.joblib")}, ensure_ascii=False))
